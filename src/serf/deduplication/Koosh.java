...
public class Koosh {
  ...
  public Set<Record> execute(MatcherMerger mm, Set<Record> records) {
    // If we're removing dominated records, create the index and add all records
    // to it
    ...
    while (!records.isEmpty()) {
      // if (i % 50 == 0)
      // System.out.println("R size: " + records.size() + ", R' size: " +
      // rprime.size());
      ...
      for (Record buddy : buddies) {
        ...
        // it is not the same as the current record, and is not already in
        // rprime
        // it is not dominated by other records (if we're removing dominated
        // records)
        if (merged.getConfidence() > _threshold && !rprime.contains(merged)
      ...
      // We've compared with everything in rprime and created all the merged
      // records, so
