...
public class ER {
  ...
  static String configFile = "example/example.conf";
  ...
  // main function reads in a property file which specifies FileSource,
  // MatcherMerger, and Algorithm(probably next step)
  public static void main(String[] args) throws Exception {
    ...
    if (checkMatcherMergerInterface(matcherMerger) != true) {
      throw (new TestException(
          "Given MatcherMerger class does not implement SimpleMatcherMerger interface!"));

    }
    runRSwoosh();
  }

  // Recursively checks if testClass or any of its ancestor class implements
  // MathcerMerger interface
  ...
  private static void runRSwoosh() throws SAXException, IOException,
    ...
    try {
      // Can we assume MatcherMerger's contructor will always take in
      ...
      Set<Record> result =
          RSwoosh.execute((serf.data.MatcherMerger) matcherMerger, records);
      System.out.println("After running RSwoosh, there are " + result.size()
          + " records.");
