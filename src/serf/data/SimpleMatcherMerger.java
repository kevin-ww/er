...
public class SimpleMatcherMerger implements MatcherMerger {
  ...
  // Defined in the Swoosh paper. k is the width of the matching range, d is the
  ...
  /**
   * This creates a SimpleMatcherMerger that checks for proximity within k, but
   * doesn't look for wraparound around a particular domain.
   * 
   * @param factory
   * @param k
   */
  ...
  /**
   * @param factory
   * @param d - the domain of possible values [1, domain]. Wraparounds will be
   *          detected and considered matches.
   * @param k - the width of the matching range. if k == 1, then only exact
   *          matches count. if k == 2 then 0 matches with -1, 0, and 1.
   */
  ...
  private boolean attrMatch(Attribute a1, Attribute a2) {
    ...
    while (i1 < a1values.length && i2 < a2values.length) {
      // while the value in a1 is lower, keep moving to the next until you find
      // one
